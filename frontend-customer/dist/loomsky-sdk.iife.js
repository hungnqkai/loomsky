    ...
    getSetupState() {
      try {
        ...
        if (sessionData && sessionData.isActive && sessionData.token) {
          ...
          return {
            isSetupMode: true,
            setupType: sessionData.setupType || "mapper",
            // Default to mapper for backward compatibility
      ...
      if (setupState.isSetupMode) {
        ...
        if (isActivated && !setupState.fromSession) {
          const sessionData = {
            ...
            // Save the setup type for navigation persistence
